...
const ArticleCreator: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (article.content_type === 'pdf' && !article.pdf_file) {
      displayNotification('error', 'PDF file is required for PDF type articles');
      return false;
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await fetchWithAuth({
        method: 'POST',
        path: '/admin/articles-create_new',
        body: formData,
        isFormData: true
      });

      displayNotification('success', 'Article created successfully!');
      navigate('/admin/article-list');
    } catch (error) {
      console.error('Error creating article:', error);
      displayNotification('error', 'Failed to create article. Please try again.');
    } finally {
    ...
    <div className="container mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {article.content_type === 'pdf' && (
          <div className="flex flex-col space-y-2">
            <label className="font-medium text-gray-700">PDF Document *</label>
            <div className="mt-1 flex items-center">
              <input
                type="file"
            ...
            {article.pdf_file && (
              <p className="text-sm text-gray-500">
                Selected file: {article.pdf_file.name}
              </p>
            )}
          </div>
        )}
        ...
        <div className="pt-4">
          <button
            ...
            className="flex items-center justify-center px-6 py-3 bg-indigo-600 text-white font-medium rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 w-full md:w-auto disabled:opacity-50"
          >
            {isSubmitting ? (
              <>
